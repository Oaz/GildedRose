       ...
       public void NewUpdateQuality()
       {
          Func<int,int> noNegative = v => (v<0) ? 0 : v;
           ...
           var behaviors = new List<Behavior>
                      {
                          new Behavior {SellInChange = (s,q) => s-1, QualityChange = (s,q) =>  (s<0) ? q-2 : q-1}, // +5 Dexterity Vest
                          ...
                          new Behavior {SellInChange = (s,q) => s-1, QualityChange = (s,q) => (s<10) ? ((s<5) ? q+3 : q+2) : q+1}, // Backstage passes to a TAFKAL80ETC concert
